...
| A3 | Reboot, unlock from laptop `keyholder unlock` | `[U]` in progress. First reboot (2026-09-18) surfaced a real bug: the agent never got a working DNS resolver in the initrd (looped indefinitely trying to reach `relays.syncthing.net` against a refusing loopback stub resolver), so it never announced itself to discovery; fixed (see `STATUS.md`). Second reboot (2026-09-19) with the fix deployed: still not announcing as of this line, still being diagnosed | console log described by a human with hypervisor access; discovery HTTP 404 for the machine's Device ID throughout |
